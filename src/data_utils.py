...
class DataLoader:
    ...
    def prepare_and_save_dataset(self):
        ...
        train_test_split = shuffled_ds.train_test_split(test_size=self.config['dataset']['test_size'])

        # Split the test set into validation and test sets
        test_valid_split = train_test_split['test'].train_test_split(test_size=self.config['dataset']['val_size'])
    ...
    def tokenize_function(self, examples):
    	...
    	tokenized_labels = self.tokenizer(examples[self.config['dataset']['response_0_column']],padding="max_length",truncation=True,max_length=self.config['model']['max_length'])
    ...
    def preprocess_for_dpo(self, dataset):
        ...
        formatted_dataset = dataset.map(preprocess_function, remove_columns=dataset['train'].column_names)
